...
export function LocationFormModal({
  ...
  // Quando o modal abre, preenche os campos: em edição usa os dados do local,
  // em criação usa os valores vazios.
  useEffect(() => {
    if (open) {
      form.resetFields()
      form.setFieldsValue(
          ...
          <Form.Item
            ...
            rules={[
              { required: true, message: 'Informe o código do local.' },
              {
                min: 2,
                max: 20,
                message: 'O código deve ter entre 2 e 20 caracteres.',
              },
              {
                pattern: /^[A-Z0-9-]+$/,
                message:
                  'Use apenas letras maiúsculas, números e hífen. Ex: LAB-03',
              },
            ]}
          >
